...
def main(kwargs, job_dir):
    ...
    executor.update_parameters(
        ...
        slurm_job_name=f"gen_train_ct_for_gelu_{pretrained_ds}_to_{transfer_ds}_{model}_seed{seed}",  # Job name
    ...
    command = (
        ...
        f"python -u generalization_trainable_ct_for_gelu.py --model {model} --pretrained_ds {pretrained_ds} --transfer_ds {transfer_ds} --seed {seed} --transfer_test_bs {transfer_test_bs}"
    )
